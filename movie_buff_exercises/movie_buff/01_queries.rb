...
def harrison_ford
  ...
  Movie.joins(:actors).where("actors.name = 'Harrison Ford' AND castings.ord != 1").select("movies.id, movies.title")
end
...
def biggest_cast
  ...
  # actors).
  Movie.joins(:castings).group(:id).order("COUNT(castings.id) DESC").select(:id, :title).limit(3)
end
